...
$currentRootPath = File.dirname(__FILE__)
$zybSpecName = "afpai-zybspecs"
$sources="--sources=https://github.com/CocoaPods/Specs.git,git@git.afpai.com:native/ZYBSpecs.git"
...
option_parser = OptionParser.new do |opts|
 ...
 $ \033[40m\033[32mruby test.rb  -p Login.podspec -t 0.0.4\033[0m
   ...
   here is help messages of the command line tool."
	...
	opts.on( "-u", "--repo-update", "Pod lib lint / pod repo push default --allow-warnings" ) do
...
module Logger
    def Logger.print(errType, msg) 
    	if  errType != nil && msg!=nil
            if errType == 'ERROR'
            	puts " \033[0m\033[31m[#{errType}] #{msg}\033[0m\n\n"
            	puts " \033[0m\033[31m[Usage: Use --help for more infomation ]\033[0m\n"
                exit
            else
            	puts " \033[0m\033[34m[#{errType}] #{msg}\033[0m\n"
            end
        end
    end
end
...
module Repo
	...
	def Repo.lint
		...
		cmdpoblib = "pod lib lint --verbose #{@uselibraries} #{@allowwarnings} --silent"
	...
	def Repo.push
		cmdcd = "cd #{$specObject.rootPath}"
		cmdpobpush = "pod repo push --silent --no-ansi #{@repoSpecs} #{$specObject.specName} #{@uselibraries} #{@allowwarnings} #{$sources} "
...
module FileManager
	def FileManager.checkPodspec
		...
		if File.exist?(podspec) ==false
			Logger.print("ERROR","#{podspec} : No such file or directory")
		end
